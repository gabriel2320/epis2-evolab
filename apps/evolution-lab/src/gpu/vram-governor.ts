    ...
    if (lastHealth.level === 'critical') {
      ...
      if (loaded.length > 0) {
        log.info('VRAM critical — descargando modelos Ollama', {
          models: loaded.map((m) => m.name),
          reasons: lastHealth.reasons,
        });
        await prepareExclusiveModel(opts.ollamaUrl, opts.model);
      }
      const waitSec = Math.max(lastHealth.cooldownSec, 30);
      log.warn(`VRAM critical — pausa ${waitSec}s`, { reasons: lastHealth.reasons });
      await sleep(waitSec * 1000);
      continue;
    }

    log.info('VRAM warn — pausa breve antes de mutación', { reasons: lastHealth.reasons });
